...
初始目录准入保持逐条串行。H1、H2、H3和延后候选终审使用固定大小的并发预审核批次，默认并发数为8；批次结果仍按原候选顺序提交。每条提交前重新计算最新已注册候选，若模型可见的召回与预审核时不同，则立即基于最新记忆同步重审一次。该策略不进行候选聚类或动态扩缩并发，可通过`--admission-workers`统一调整后四轮并发数。
